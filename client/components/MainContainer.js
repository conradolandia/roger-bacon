...
export default function MainContainer() {
  ...
  const askAI = async () => {
    if (question === '') {
      toast.error('Por favor ingrese un input válido e intente nuevamente.');
    } else {
      setLoading(true);
          ...
          {chat.length > 0 ? (
            chat.map(msg => (
              <Stack direction="vertical" gap={3}>
                ...
                >
                  <div>
                    {msg.isBot ? (
                      <span style={{ fontSize: '12px' }}>Roger Bacon</span>
                    ) : (
            ...
            >
              <h1 className="mt-sm-5">
                <small>Roger Bacon: Un proyecto de Pendragon RHM</small>
              </h1>
              <h5 style={{ color: 'gray' }}>Basado en PrivateGPT</h5>
            </Stack>
          )}
          {loading ? (
